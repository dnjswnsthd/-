...
// 프로그래머스 코딩테스트 연습 수식 최대화

public class Pro_maxCal {
    static char[] prior = {'+', '-', '*'};
    ...
    public static Long operation(Long num1, Long num2, char op){
        long num = 0;
        switch (op){
            case '+' : {
                return num1 + num2;
        ...
        if (count == 3) {
            ...
            for (int i = 0; i < p.length; i++) {
                for (int j = 0; j < cOps.size(); j++) {
                    if (p[i] == cOps.get(j)) {
                        Long res = operation(cNums.remove(j), cNums.remove(j), p[i]);
                        cNums.add(j, res);
                        cOps.remove(j);
                        j--;
                    }
